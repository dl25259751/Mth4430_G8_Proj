...
for(fold in 1:5){
  ...
  print(lasso_high$lambda.min)
}

lasso_log_loss <- mean(lasso_cv_log_loss)
```

##LASSO refit with lambdamins
```{r}
lambda_min <- c(lasso_low$lambda.min, lasso_moderate$lambda.min, lasso_high$lambda.min)

X_tr_full_matrix <- as.matrix(tr[, -c(13, 14, 15, 16)])
Y_tr_full_targets <- tr[, c(14, 15, 16)]

best_lasso_high <- glmnet(X_tr_full_matrix, tr[,16 ], family = "binomial", type.measure = "deviance", lambda = lambda_min[3])
best_lasso_moderate <- glmnet(X_tr_full_matrix, tr[,15 ], family = "binomial", type.measure = "deviance", lambda = lambda_min[2])
best_lasso_low <- glmnet(X_tr_full_matrix, tr[,14 ], family = "binomial", type.measure = "deviance", lambda = lambda_min[1])

best_lasso_high_pred <- predict(best_lasso_high, newx = X_tr_full_matrix, s = lambda_min[3], type = "response")
best_lasso_moderate_pred <- predict(best_lasso_moderate, newx = X_tr_full_matrix, s = lambda_min[2], type = "response")
best_lasso_low_pred <- predict(best_lasso_low, newx = X_tr_full_matrix, s = lambda_min[1], type = "response")
