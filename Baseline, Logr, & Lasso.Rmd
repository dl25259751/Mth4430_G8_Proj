...
## some data vis stuff
```{r}
#scatter plot of blood pressure health data
ggplot(data = student_health_data, aes(x = Blood_Pressure_Systolic, y = Blood_Pressure_Diastolic, color = Health_Risk_Level)) +
  geom_point() +                          # Draws the points
  labs(title = "Blood_Pressure_Systolic ~ Blood_Pressure_Diastolic",
...
```
```{r}
#grped bar chart for phys act, a strong indicator according to lasso & logr coefs
ggplot(student_health_data, aes(x = Physical_Activity, fill = Health_Risk_Level)) +
  geom_bar(position = "dodge") +  # "stack" is actually the default
  labs(title = "Dist of Health_Risk_Level by Physical_activity",
       x = "Physical_Activity",
       y = "Count",
       fill = "Class") +
  theme_minimal()                 
```
```{r}
#grped bar chart for sleep qual, a strong indicator according to lasso & logr coefs
ggplot(student_health_data, aes(x = Sleep_Quality, fill = Health_Risk_Level)) +
...
```
```{r}
#stress levels scatter plot, second strongest predictors when viewed from the lens of pure magnitude
ggplot(data = student_health_data, aes(x = Stress_Level_Biosensor, y = Stress_Level_Self_Report, color = Health_Risk_Level)) +
  geom_point() +                          
  labs(title = "Stress_Level_Biosensor ~ Stress_Level_Self_Report",
...
```
```{r}
#school and excternal work scatter plot
ggplot(data = student_health_data, aes(x = Project_Hours, y = Study_Hours, color = Health_Risk_Level)) +
  geom_point() +                          
  labs(title = "Project_Hours ~ Study_Hours",
       x = "Project_Hours",
       y = "Study_Hours",
       color = "Class") +                 
  theme_minimal()  
...
final_logr_Low$coefficients
final_logr_Moderate$coefficients
final_logr_High$coefficients
```
